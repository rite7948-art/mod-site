...
<body>
    ...
    <script>
        ...
        const mastersInterviewData = [
            ...
            { q: 'Пользователь отправил ссылку на сайт с мемами. Ссылка была под спойлером. На сайте нет рекламы, но есть политические картинки и пара мемов с запрещённой символикой. Какие правила нарушены и какое наказание?', a: 'Сразу два правила: 1.1 (размещение сторонних ресурсов без согласования) и 1.4 (политический контент + запрещённая символика). Наказание: варн / бан навсегда без амнистии.' },
            { q: 'Пользователь во время мута за флуд зашёл с твинка и в приватной комнате использовал SoundPad с громкими звуками. Нарушает ли он правила, если приватки не модерируются?', a: 'Нарушает. Использование твинка для обхода наказания — правило 1.6. SoundPad в приватке сам по себе не нарушение, но факт присутствия на твинке во время активного наказания уже фиксируется.' },
            { q: 'Если пользователь написал в чат «иди кури», имея в виду обычные сигареты (не наркотики), это нарушение?', a: 'Да. По правилу 1.4 запрещена пропаганда курения, даже если речь про обычные сигареты.' },
            { q: 'Пользователь отправил каждому подряд по одному упоминанию через каждые 10 минут. Это считается флудом / спамом?', a: 'Это не флуд, но по правилу 1.8 «беспричинное многократное упоминание участников и ролей» — отдельный пункт. Мут на 4 часа.' },
            { q: 'Участник написал: «Ты меня уважаешь, или мне к твоим фоткам обратиться?» — имел в виду мем, фоток у него нет. Нарушение?', a: 'Да. Даже косвенные / двусмысленные угрозы доксом (правило 1.3) — нарушение, независимо от реальности угрозы.' },
            ...
            { q: 'Игрок поставил ник «Sanya_FSB» и баннер с картинкой из фильма про войну. В чате ничего политического не пишет. Нарушение?', a: 'Да. По правилу 1.5 запрещено политическое / провокационное содержимое профиля (ник + баннер), даже если в чате молчит.' },
            ...
            { q: 'Администрация выдала бан на 1 день «для профилактики», хотя в правилах указано 30 дней / навсегда. Это легально?', a: 'Да. В примечаниях: «администрация может наказывать пользователей на своё усмотрение без объяснения причин» — перекрывает стандартные сроки.' },
            { q: 'Участник включил вебку и сидит без микрофона. На камере виден флаг страны. Нарушает ли он правила?', a: 'Да. По правилу 1.4 запрещено использование флагов стран и политической символики — даже на вебке, а не только в чате.' },
            { q: 'Пользователь транслирует с вебки, у него на фоне алкогольные напитки. Сам ничего не пьёт и не рекламирует. Нарушение?', a: 'Да. По правилу 1.4 запрещена пропаганда алкоголя; видимость алкоголя на фоне расценивается как косвенная демонстрация.' },
            { q: 'Пользователь на вебке включил ролик с изображением сцен охоты (убийство животных). Это нарушение?', a: 'Да. По правилу 1.4 запрещён шокирующий / тошнотворный контент. Демонстрация убийства животных подпадает сюда, даже если «документальное видео».' },
            { q: 'Участник на вебке показал игрушечный пистолет и сказал «вот сейчас я вас всех завалю» в шутку. Нарушение?', a: 'Да. По правилу 1.3 — даже косвенные / завуалированные угрозы. Плюс может трактоваться как провокационный контент (правило 1.4).' },
            { q: 'Пользователь в голосовом канале без вебки говорит: «Я сегодня утопил свою кошку, ха-ха». Никто не видел, правда это или нет. Нарушение?', a: 'Да. Подпадает под шокирующий / тошнотворный контент (правило 1.4), даже если это ложь.' },
            { q: 'Участник во время стрима с вебкой сидит в маске и издаёт нечленораздельные крики. Это нарушение?', a: 'Да. По правилу 1.10 — неадекватное поведение (крики, провокации), даже если это «шутка».' },
            { q: 'Пользователь демонстрирует на вебке шокирующий контент (например, видео аварии), но делает это в приватной комнате. Нарушение?', a: 'Да. В отличие от мата, оскорблений и флуда — шокирующий контент (правило 1.4) запрещён в любом месте, включая приватные комнаты.' },
            { q: 'Участник включает вебку и просто сидит голым по пояс. Нарушение, если ничего неприличного не показывает?', a: 'Да. Может быть классифицировано как сексуальный / провокационный контент (правило 1.4). Даже «по пояс» — повод для жалобы.' },
            { q: 'Пользователь на вебке показал домашнего питомца, но в шутку сказал «сейчас я его придушу». Самого насилия не было. Нарушение?', a: 'Да. По правилу 1.3 (косвенные угрозы) + правилу 1.4 (пропаганда насилия, шокирующий контент). «Шутка» трактуется как нарушение.' },
            { q: 'Во время вебки пользователь показывает запрещённую символику на одежде (например, свастику), но сам ничего не говорит. Это нарушение?', a: 'Да. По правилу 1.4 прямо запрещена демонстрация запрещённой символики — независимо от того, говорил ли участник что-то.' }
        ];
